...
async function main() {
  ...
  // Y-Left
  const vertexSets1 = [];
  const points1 = Matter.Svg.pathToVertices(
    svgEl.getElementsByTagName('path')[0],
    50
  );
  ...
  // YO-middle
  const vertexSets2 = [];
  const points2 = Matter.Svg.pathToVertices(
    svgEl.getElementsByTagName('path')[1],
    50
  );
  ...
  // YO-middle-inverse
  const vertexSets3 = [];
  const points3 = Matter.Svg.pathToVertices(
    svgEl.getElementsByTagName('path')[2],
    50
  );
  vertexSets3.push(points3);
  const body3 = Matter.Bodies.fromVertices(
    offset.x + 822,
    offset.y + 261,
    vertexSets3,
    { isStatic: true }
  ...
  // O-inner
  const vertexSets4 = [];
  const points4 = Matter.Svg.pathToVertices(
    svgEl.getElementsByTagName('path')[3],
    20
  );
  vertexSets4.push(points4);
  const body4 = Matter.Bodies.fromVertices(0, 0, vertexSets4, {
    isStatic: true,
  });
  Matter.World.add(engine.world, body4);
  Matter.Body.setPosition(
    body4,
    Matter.Vector.create(offset.x + 620, offset.y + 300)
  );
  ...
  Promise.all(addParticleTasks).then(() => {
    gravityNoise = true;
  });
...
async function addParticles(count, delay, x, y) {
...
function animate() {
  ...
  if (gravityNoise) {
    ...
    engine.world.gravity.x = noise.perlin2(10, i) * NOISE_FACTOR;
    engine.world.gravity.y = noise.perlin2(1, i) * NOISE_FACTOR;
    frameCount += 1;
  }
