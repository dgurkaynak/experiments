...
async function main() {
  ...
  viewSettings
    .add(settings, 'sphereRadius', 0.01, 0.1)
    .step(0.005)
    .onChange((val) => {
      meshes.forEach((line) =>
        line.forEach((mesh) => mesh.scale.set(val, val, val))
      );
    });
  viewSettings
    .addColor(settings, 'bgColor')
    .listen()
    .onChange(setBackgroundAndFogColor);
  viewSettings
    .addColor(settings, 'sphereColor')
    .listen()
 ...
 */
function setBackgroundAndFogColor(color) {
