...
export default class TextPhysics extends ExperimentTwoJs {
  ...
  particles: [Matter.Body, Two.Shape][] = [];
  frameCount = 0;
  gravityNoise = false;
  ...
  async init() {
    ...
    Matter.World.add(this.engine.world, body1);
    ...
    Matter.World.add(this.engine.world, body2);
    ...
    Matter.World.add(this.engine.world, body3);
    ...
    Matter.World.add(this.engine.world, body4);
    Matter.Body.setPosition(body4, Matter.Vector.create(offset.x + 620, offset.y + 300));

    const addParticleTasks = [
      this.addParticles(PARTICLE_COUNT.Y / 2, 50, offset.x + 100, offset.y + 150),
      this.addParticles(PARTICLE_COUNT.Y / 2, 50, offset.x + 350, offset.y + 150),
      this.addParticles(PARTICLE_COUNT.O / 2, 50, offset.x + 500, offset.y + 150),
      this.addParticles(PARTICLE_COUNT.O / 2, 50, offset.x + 750, offset.y + 150)
    ];
    Promise
      .all(addParticleTasks)
      .then(() =>{
        this.gravityNoise = true;
      });
  ...
  async addParticles(count: number, delay: number, x: number, y: number) {
  ...
  addParticle(x: number, y: number) {
    ...
    const view = this.two.makeCircle(x, y, radius);
    view.noStroke();
    view.fill = COLORS.PARTICLE;

    this.particles.push([body, view]);
  }


  updateParticles() {
    this.particles.forEach(([body, shape]) => {
  ...
  requestAnimationFrame() {
    if (this.gravityNoise) {
      const i = this.frameCount * 0.01;
      this.engine.world.gravity.x = noise.noise.perlin2(10, i) * NOISE_FACTOR;
      this.engine.world.gravity.y = noise.noise.perlin2(1, i) * NOISE_FACTOR;
      this.frameCount += 1;
    }
